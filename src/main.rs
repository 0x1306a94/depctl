...
use anyhow::Result;
use tasks::{CleanTask, DepsTask, Task};
...
fn main() -> Result<()> {
    ...
    if options.version {
        ...
        if git_hash != "unknown" && git_url != "unknown" {
            println!(
                "depctl version {} (commit {}, {}, built {})",
                version, short_hash, git_url, build_time
            );
        } else if git_hash != "unknown" {
            println!(
                "depctl version {} (commit {}, built {})",
                version, short_hash, build_time
            );
    ...
    let config_file = if let Some(project) = &options.project {
        ...
        if !deps_file.exists() {
            eprintln!(
                "Cannot find {} at the specified directory: {}",
                filename,
                project.display()
            );
            std::process::exit(1);
    ...
    if options.clean {
        ...
        // 在所有同步完成后，按入栈顺序执行
        let post_sync_stack =
            std::sync::Arc::new(std::sync::Mutex::new(Vec::<Box<dyn tasks::Task>>::new()));
