    ...
    fn run(&self) -> Result<bool> {
        ...
        if is_symlink {
            ...
            #[cfg(unix)]
            {
                ...
                let link_target_path = if link_target.is_absolute() {
                    ...
                    if let Some(dest_parent) = self.item.dest.parent() {
                        pathdiff::diff_paths(&link_target_abs, dest_parent).unwrap_or_else(|| {
                            // 如果无法计算相对路径，使用绝对路径
                            link_target_abs
                        })
                    } else {
                ...
                if target_metadata.is_dir() {
                    // 创建目录软链接
                    symlink(&link_target_path, &self.item.dest).with_context(|| {
                        format!(
                            "Failed to create directory symlink: {} -> {}",
                            self.item.dest.display(),
            ...
            #[cfg(not(unix))]
            {
                ...
                utils::log(&format!(
                    "【depctl】copied (Windows fallback, symlink content): {} -> {}",
                    self.item.src.display(),
                    self.item.dest.display()
                ));
            }
        } else {
            ...
            if src_metadata.is_dir() {
                ...
                copy_dir_all(&self.item.src, &self.item.dest)?;
                utils::log(&format!(
                    "【depctl】copied directory: {} -> {}",
                    self.item.src.display(),
                    self.item.dest.display()
                ));
            } else {
                // 复制文件
                std::fs::copy(&self.item.src, &self.item.dest).with_context(|| {
                    format!(
                        "Failed to copy file: {} -> {}",
                        self.item.src.display(),
                        self.item.dest.display()
            ...
            #[cfg(unix)]
            {
                ...
                let link_target_path = if link_target.is_absolute() {
                    ...
                    if let Some(dst_parent) = dst_path.parent() {
                        pathdiff::diff_paths(&link_target_abs, dst_parent).unwrap_or_else(|| {
                            // 如果无法计算相对路径，使用绝对路径
                            link_target_abs
                        })
                    } else {
                ...
                if target_metadata.is_dir() {
                    // 创建目录软链接
                    symlink(&link_target_path, &dst_path).with_context(|| {
                        format!(
                            "Failed to create directory symlink: {} -> {}",
                            dst_path.display(),
                            link_target_path.display()
