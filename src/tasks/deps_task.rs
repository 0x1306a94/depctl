...
use crate::tasks::{
    ActionTask, CopyFileTask, FileTask, LinkFileTask, RepoTask, SubRepoTask, Task, TaskRunner,
};
use crate::utils;
use anyhow::Result;
...
impl Task for DepsTask {
    fn run(&self) -> Result<bool> {
        ...
        let unfinish_file_in_git = self
            .config_file
            .parent()
            .unwrap()
            .join(".git")
            .join(".DEPS.unfinished");
        ...
        if let Some(ref stack) = self.post_sync_stack {
            for item in &config.linkfiles {
                ...
                stack_guard.push(Box::new(LinkFileTask::new(
                    item.clone(),
                    self.force_linkfiles,
                )));
            }
            ...
            // 如果没有栈，直接执行（向后兼容）
            for item in &config.linkfiles {
                tasks.push(Box::new(LinkFileTask::new(
                    item.clone(),
                    self.force_linkfiles,
                )));
            }
        ...
        if let Some(ref stack) = self.post_sync_stack {
            for item in &config.copyfiles {
                let mut stack_guard = stack.lock().unwrap();
                stack_guard.push(Box::new(CopyFileTask::new(
                    item.clone(),
                    self.force_copyfiles,
                )));
            }
            ...
            // 如果没有栈，直接执行（向后兼容）
            for item in &config.copyfiles {
                tasks.push(Box::new(CopyFileTask::new(
                    item.clone(),
                    self.force_copyfiles,
                )));
            }
