    ...
    fn run(&self) -> Result<bool> {
        ...
        #[cfg(unix)]
        {
            ...
            let link_target = if let Some(dest_parent) = self.item.dest.parent() {
                ...
                pathdiff::diff_paths(&self.item.src, dest_parent).unwrap_or_else(|| {
                    // 如果无法计算相对路径，使用绝对路径
                    std::fs::canonicalize(&self.item.src).unwrap_or_else(|_| self.item.src.clone())
                })
            } else {
                // 没有父目录，使用绝对路径
                std::fs::canonicalize(&self.item.src).unwrap_or_else(|_| self.item.src.clone())
            ...
            if src_metadata.is_dir() {
                // 创建目录软链接
                symlink(&link_target, &self.item.dest).with_context(|| {
                    format!(
        ...
        #[cfg(not(unix))]
        {
            ...
            utils::log(&format!(
                "【depctl】copied (Windows fallback): {} -> {}",
                self.item.dest.display(),
