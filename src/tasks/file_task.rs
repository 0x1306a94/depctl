        ...
        for entry_result in archive.entries()? {
            ...
            if let Ok(Some(link_target)) = entry.link_name() {
                ...
                #[cfg(unix)]
                {
                    ...
                    symlink(link_target.as_ref(), &target_path).with_context(|| {
                        format!(
                            "Failed to create symlink: {} -> {}",
                            target_path.display(),
                            link_target.display()
...
impl Task for FileTask {
    fn run(&self) -> Result<bool> {
        let url_without_query: String = self
            .item
            .url
            .split('?')
            .next()
            .unwrap_or(&self.item.url)
            .to_string();
            ...
            for (index, suffix) in multipart.iter().enumerate() {
                ...
                let mut file = if index == 0 {
                    ...
                    File::options()
                        ...
                        .open(&file_path)
                        .with_context(|| {
                            format!("Failed to open file for appending: {}", file_path.display())
                        })?
                };
